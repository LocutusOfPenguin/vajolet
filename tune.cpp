...
double Tuner::parseEpd()
{
	...
	//sync_cout << "opening epd" << sync_endl;
	...
	do
	{
		...
		if( !fenstr.empty() )
		{
			...
			std::size_t pos = fenstr.find(" ");
			std::string res = fenstr.substr(0,pos);
			std::string fen = fenstr.substr(pos+1);
			bool color = fen.find(" b ") != std::string::npos;

			double fRes = std::stod(res);

			searcher.pos.setupFromFen(fen);

			std::list<Move> childPV;
			Score score = searcher.qsearch<search::nodeType::PV_NODE>(0,0,-SCORE_INFINITE,SCORE_INFINITE,childPV);

			if(color) score*= -1;
			double error = std::pow(fRes - (1.0 / ( 1.0 + std::pow(2.71828182846, -score / scaling) ) ), 2);
			sum += error;

			//sync_cout << fRes <<" " <<color<<" "<<score<<" "<<error<< sync_endl;
		}
...
void Tuner::FindBestScaling()
{

	for(scaling = 1000;scaling <300000; scaling +=1000)
		Tuner::parseEpd();
}
