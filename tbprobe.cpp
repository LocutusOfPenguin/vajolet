...
struct PairsData {
    ...
    Position::bitboardIndex pieces[TBPIECES];        // Position pieces: the order of pieces defines the groups
...
int MapA1D1D4[squareNumber];
int MapKK[10][squareNumber]; // [MapA1D1D4][SQUARE_NB]
...
const std::string PieceToChar = " KQRBNP  kqrbnp";

int Binomial[6][squareNumber];    // [k][n] k elements from a set of n elements
int LeadPawnIdx[5][squareNumber]; // [leadPawnsCnt][SQUARE_NB]
...
class HashTable {
    ...
    typedef std::pair<U64, EntryPair> Entry;
    ...
    void insert(U64 key, WDLEntry* wdl, DTZEntry* dtz) {
...
public:
  ...
  void insert(const std::vector<Position::bitboardIndex>& pieces);
};
...
WDLEntry::WDLEntry(const std::string& code) {
    ...
    key = pos.getMaterialKey();
    pieceCount = bitCnt(pos.getOccupationBitmap());
    hasPawns = pos.getBitmap(Position::whitePawns) | pos.getBitmap(Position::blackPawns);

    for (Position::bitboardIndex pt =Position::occupiedSquares; pt < Position::lastBitboard; pt = (Position::bitboardIndex)(pt+1))
    {
    	if(		pt != Position::occupiedSquares &&
    			pt != Position::whiteKing &&
				pt != Position::whitePieces &&
				pt != Position::separationBitmap &&
				pt != Position::blackKing &&
				pt != Position::blackPieces
    	)
    	{
    		if (pos.getPieceCount(pt) == 1)
    		{
    ...
    if (hasPawns)
    {
        ...
        bool c =   !pos.getPieceCount(Position::blackPawns)
                || (   pos.getPieceCount(Position::whitePawns)
                    && pos.getPieceCount(Position::blackPawns) >= pos.getPieceCount(Position::whitePawns));

        pawnTable.pawnCount[0] = pos.getPieceCount(c ? Position::whitePawns : Position::blackPawns);
        pawnTable.pawnCount[1] = pos.getPieceCount(c ? Position::blackPawns : Position::whitePawns);
    }

    pos.setup(code, black);
    key2 = pos.getMaterialKey();
}
...
WDLEntry::~WDLEntry() {
    ...
    for (int i = 0; i < 2; ++i)
        if (hasPawns)
            for (int f = FILES[A1]; f <= FILES[D1]; ++f)
...
void HashTable::insert(const std::vector<Position::bitboardIndex>& pieces) {

    std::string code;

    for (Position::bitboardIndex pt : pieces)
        code += PieceToChar[pt];
...
bool check_dtz_stm(WDLEntry*, int, int) { return true; }

bool check_dtz_stm(DTZEntry* entry, int stm, int f) {
...
WDLScore map_score(WDLEntry*, int, int value, WDLScore) { return WDLScore(value - 2); }

int map_score(DTZEntry* entry, int f, int value, WDLScore wdl) {
...
T do_probe_table(const Position& pos,  Entry* entry, WDLScore wdl, ProbeState* result) {
    ...
    Position::bitboardIndex pieces[TBPIECES];
    uint64_t idx;
    ...
    bool symmetricBlackToMove = (entry->key == entry->key2 && pos.getActualState().nextMove);
    ...
    bool blackStronger = (pos.getMaterialKey() != entry->key);

    std::cout<<"symm "<<symmetricBlackToMove<<std::endl;
    std::cout<<"black "<<blackStronger<<std::endl;

    int flipColor   = (symmetricBlackToMove || blackStronger) * 8;
    int flipSquares = (symmetricBlackToMove || blackStronger) * 070;
    int stm         = (symmetricBlackToMove || blackStronger) ^ (pos.getActualState().nextMove == Position::blackTurn);

    std::cout<<"flip1 "<<flipColor<<std::endl;
    std::cout<<"flip2 "<<flipSquares<<std::endl;
    ...
    if (entry->hasPawns) {
    	...
    	std::cout<<"haspawns"<<std::endl;
        // In all the 4 tables, pawns are at the beginning of the piece sequence and
        // their color is the reference one. So we just pick the first one.
    	Position::bitboardIndex pc = (Position::bitboardIndex)(Position::bitboardIndex(item(entry->pawnTable, 0, 0).precomp->pieces[0] ^ flipColor)+5);
    	std::cout<<"pc "<<pc<<std::endl;
        assert( pc == Position::whitePawns || pc == Position::blackPawns);
    ...
    while (b) {
        ...
        pieces[size++] = Position::bitboardIndex(pos.getPieceAt(s) ^ flipColor);
    }
...
template<typename T>
void set_groups(T& e, PairsData* d, int order[], int f) {
...
template<typename T>
uint8_t* set_dtz_map(WDLEntry&, T&, uint8_t*, int) { return nullptr; }

template<typename T>
uint8_t* set_dtz_map(DTZEntry&, T& p, uint8_t* data, int maxFile) {

    p.map = data;

    for (int f = FILES[A1]; f <= maxFile; ++f) {
...
void do_init(Entry& e, T& p, uint8_t* data) {
    ...
    const int MaxFile = e.hasPawns ? FILES[D1] : FILES[A1];
    ...
    for (int f = FILES[A1]; f <= MaxFile; ++f) {
        ...
        for (int k = 0; k < e.pieceCount; ++k, ++data)
            for (int i = 0; i < Sides; i++)
                item(p, i, f).precomp->pieces[k] = Position::bitboardIndex(i ? *data >>  4 : *data & 0xF);
...
void* init(Entry& e, const Position& pos) {
    ...
    for (Position::bitboardIndex pt = Position::King; pt <= Position::Pawns; pt = (Position::bitboardIndex)(pt+1)) {
        w += std::string(pos.getPieceCount(pt), PieceToChar[pt]);
        b += std::string(pos.getPieceCount((Position::bitboardIndex)(pt + Position::separationBitmap)), PieceToChar[pt]);
    }

    const uint8_t TB_MAGIC[][4] = { { 0xD7, 0x66, 0x0C, 0xA5 },
                                    { 0x71, 0xE8, 0x23, 0x5D } };

    fname =  (e.key == pos.getMaterialKey() ? w + 'v' + b : b + 'v' + w)
           + (IsWDL ? ".rtbw" : ".rtbz");
...
T probe_table(const Position& pos, ProbeState* result, WDLScore wdl = WDLDraw) {
    ...
    if (!(pos.getOccupationBitmap() ^ pos.getBitmap(Position::whiteKing) ^ pos.getBitmap(Position::blackKing) ))
        return T(WDLDraw); // KvK

    E* entry = EntryTable.get<E>(pos.getMaterialKey());
...
WDLScore search(Position& pos, ProbeState* result) {
    ...
    Movegen mg(pos);
    size_t totalCount = mg.getNumberOfLegalMoves(), moveCount = 0;

    Movegen mg2(pos);
    Move move;
    while ((move = mg2.getNextMove()).packed)
    {
    	sync_cout<<"test2"<<sync_endl;
    	Position::bitboardIndex piece = pos.getPieceAt((tSquare)move.bit.from);
        if (   !pos.isCaptureMove(move)
            && (!CheckZeroingMoves || !pos.isPawn(piece) ))
            continue;
    ...
    std::cout<<"mc "<<moveCount<<std::endl;
    std::cout<<"bv "<<bestValue<<std::endl;
    bool noMoreMoves = (moveCount && moveCount == totalCount);
    if (noMoreMoves)
    {
    	std::cout<<"noMoreMove"<<std::endl;
    	value = bestValue;
    }
    else
    {
...
void Tablebases::init(const std::string& paths) {
    ...
    int code = 0;
    for (tSquare s = A1; s <= H8; s = (tSquare)(s+1))
        if (off_A1H8(s) < 0)
            MapB1H1H7[s] = code++;
    ...
    code = 0;
    for (tSquare s = A1; s <= D4; s = (tSquare)(s+1))
        if (off_A1H8(s) < 0 && FILES[s] <= FILES[D1])
            MapA1D1D4[s] = code++;

        else if (!off_A1H8(s) && FILES[s] <= FILES[D1])
    ...
    for (int idx = 0; idx < 10; idx++)
        for (tSquare s1 = A1; s1 <= D4; s1 = (tSquare)(s1+1))
            if (MapA1D1D4[s1] == idx && (idx || s1 == B1)){ // SQ_B1 is mapped to 0
                for (tSquare s2 = A1; s2 <= H8; s2 = (tSquare)(s2+1))
                    if ((Movegen::attackFrom<Position::whiteKing>(s1) | bitSet(s1)) & bitSet(s2))
    ...
    for (int leadPawnsCnt = 1; leadPawnsCnt <= 4; ++leadPawnsCnt)
        for (int f = FILES[A1]; f <= FILES[D1]; ++f)
        {
            ...
            for (int r = RANKS[A2]; r <= RANKS[A7]; ++r)
            {
    ...
    for (Position::bitboardIndex p1 = Position::Pawns; p1 >= Position::King; p1= (Position::bitboardIndex)(p1-1)) {
        EntryTable.insert({Position::King, p1, Position::King});

        for (Position::bitboardIndex p2 = Position::Pawns; p2 >= p1; p2= (Position::bitboardIndex)(p2-1)) {
            EntryTable.insert({Position::King, p1, p2, Position::King});
            EntryTable.insert({Position::King, p1, Position::King, p2});

            for (Position::bitboardIndex p3 = Position::Pawns; p3 > Position::King; p3 = (Position::bitboardIndex)(p3-1))
                EntryTable.insert({Position::King, p1, p2, Position::King, p3});

            for (Position::bitboardIndex p3 = Position::Pawns; p3 >= p2; p3 = (Position::bitboardIndex)(p3-1)) {
                EntryTable.insert({Position::King, p1, p2, p3, Position::King});

                for (Position::bitboardIndex p4 = Position::Pawns; p4 >= p3; p4 = (Position::bitboardIndex)(p4-1))
                    EntryTable.insert({Position::King, p1, p2, p3, p4, Position::King});

                for (Position::bitboardIndex p4 = Position::Pawns; p4 > Position::King; p4 = (Position::bitboardIndex)(p4-1))
                    EntryTable.insert({Position::King, p1, p2, p3, Position::King, p4});
            }

            for (Position::bitboardIndex p3 = Position::Pawns; p3 >= p1; p3 = (Position::bitboardIndex)(p3-1))
                for (Position::bitboardIndex p4 = Position::Pawns; p4 >= (p1 == p3 ? p2 : p3); p4 = (Position::bitboardIndex)(p4-1))
                    EntryTable.insert({Position::King, p1, p2, Position::King, p3, p4});
        }
    }

    std::cout << "info string Found " << EntryTable.size() << " tablebases" << std::endl;
}
...
int Tablebases::probe_dtz(Position& pos, ProbeState* result) {
    ...
    *result = OK;
    ...
    Movegen mg(pos);
    Move move;
    while ((move = mg.getNextMove()).packed)
    {
    	Position::bitboardIndex piece = pos.getPieceAt((tSquare)move.bit.from);
        bool zeroing = (pos.isCaptureMove(move) || pos.isPawn(piece) );
