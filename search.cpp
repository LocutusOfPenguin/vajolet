		...
		for (indexPV = 0; indexPV < PVSize; indexPV++)
		{
			...
			do{
				...
				if(depth!=1 && signals.stop){
					...
					//sync_cout<<"DEBUG STOP depth "<< depth<<" time "<<now-startTime<<sync_endl;
					break;
				}
				...
				//sync_cout<<"DEBUG depth "<< depth<<" time "<<now-startTime<< " pv "<<p.displayUci(rootMoves[indexPV].PV.list[0]) <<sync_endl;
